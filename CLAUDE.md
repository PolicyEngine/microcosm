...
the legacy source block; its own code calls no download or staging path (the
consumer runtime's own network use is part of that runtime's qualification);
and it never writes `release_manifest.json`.
