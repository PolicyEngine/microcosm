...
bundle, a calibration or a latest pointer. The verifier itself
(:func:`microcosm.data.h5_boolean_append.compare_boolean_append`) lives in the
data shard so the reported-receipt source-enrichment release can replay it; see
...
# The multi-column verifier and what the writer below shares with it (expected
# table attributes, registration rewrite, supported-table rules) live in the
# data shard, because the reported-receipt source-enrichment release contract
# replays the same verifier before certifying or publishing this tool's child.
from microcosm.data.h5_boolean_append import (  # noqa: PLC2701
    _CHUNK_ROWS,
    _REGISTRATION,
    BooleanAppendError,
    _check_supported_table,
    _new_table_attributes,
    _registration_value,
    compare_boolean_append,
)

# Byte-level HDF primitives from the reviewed enrichment writer. They are
# generic (raw typed reads, attribute copies); only that module's role-pinned
# wrappers are unusable here, so the primitives are reused rather than
# re-derived. Their file digests are recorded in the receipt.
...
#: A refusal. Its message never contains a row value or identifier. The
#: Boolean-append verifier, which the source-enrichment release contract
#: replays, raises this same class, so a refusal from the shared verifier and
#: one from this tool are one type.
DonorQualificationError = BooleanAppendError
