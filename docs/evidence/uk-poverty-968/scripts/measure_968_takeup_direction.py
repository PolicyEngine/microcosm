...
def run(claim,label):
    sim=Microsimulation(dataset=PATH); sim.set_input("would_claim_uc",YEAR,claim.astype(bool))
    ...
    r={"label":label,"uc_families_m":float(bw[uc_b>0].sum()/1e6),"uc_bn":float((uc_b*bw).sum()/1e9),"rel_bhc_pct":100*float(pw[eq<0.6*med].sum()/pw.sum()),
       "abs_bhc_pct":100*float(pw[eq<thr].sum()/pw.sum()),"child_rel_pct":100*float(pw[child&(eq<0.6*med)].sum()/pw[child].sum()),"median_week":med/52}
    print(json.dumps(r),flush=True); return r,uc_b
res=[]; r0,_=run(base_claim,"baseline draw (v20 file)"); res.append(r0)
r_all,uc_all=run(np.ones(len(bid),bool),"everyone entitled claims"); res.append(r_all)
entitled=uc_all>0; pool=entitled&~anchor; n=int((base_claim&pool).sum())
order=np.argsort(-uc_all); pool_order=[i for i in order if pool[i]]
largest=np.zeros(len(bid),bool); largest[pool_order[:n]]=True
smallest=np.zeros(len(bid),bool); smallest[pool_order[-n:]]=True
res.append(run(anchor|largest|(base_claim&~pool),"residual to the LARGEST entitlements (DWP take-up pattern)")[0])
res.append(run(anchor|smallest|(base_claim&~pool),"residual to the SMALLEST entitlements (opposite)")[0])
# mean entitlement of the pool by chosen set
w_b=None
json.dump(res,open(sys.argv[1],"w"),indent=1); print("pool",int(pool.sum()),"n",n,"DONE")
