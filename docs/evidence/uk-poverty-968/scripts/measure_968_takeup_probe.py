...
first_p = (
    pd.Series(np.arange(len(person)))
    .groupby(b_of_p)
    .first()
    .reindex(np.arange(len(bid)))
    .to_numpy()
)
b_hh = p_idx[first_p]
...
def run(claim, label):
    ...
    r = {
        "label": label,
        "claim_true_units": int(claim.sum()),
...
r_all, uc_all, eq_all = run(
    np.ones(len(bid), bool), "everyone eligible claims (full take-up)"
)
res.append(r_all)
entitled = uc_all > 0
...
for seed in (1, 2, 3):
    ...
    pick = np.zeros(len(bid), bool)
    idx = np.flatnonzero(pool)
    pick[rng.choice(idx, size=n_base_pool, replace=False)] = True
    res.append(
        run(
            anchor | pick | (base_claim & ~pool),
            f"alternative random draw seed {seed} (same pool count)",
        )[0]
    )
...
hh_level_eq_full = np.zeros(len(hid))
hh_level_eq_full[p_idx] = eq_all
with_uc_removed = hh_level_eq_full[b_hh] - uc_all / np.maximum(
    1e-9, 1
)  # crude: subtract unit UC (not equivalised) — ordering only
order = np.argsort(with_uc_removed)
pool_order = [i for i in order if pool[i]]
poorest = np.zeros(len(bid), bool)
poorest[pool_order[:n_base_pool]] = True
richest = np.zeros(len(bid), bool)
richest[pool_order[-n_base_pool:]] = True
res.append(
    run(
        anchor | poorest | (base_claim & ~pool),
        "residual to the POOREST entitled units (lower bound)",
    )[0]
)
res.append(
    run(
        anchor | richest | (base_claim & ~pool),
        "residual to the RICHEST entitled units (upper bound)",
    )[0]
)
res.append(
    run(anchor | (base_claim & ~pool), "anchored reporters only (no residual draw)")[0]
)
