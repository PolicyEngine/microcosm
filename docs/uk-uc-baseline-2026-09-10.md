...
- Carer element: the engine's carer condition now holds on Carer's Allowance receipt or 35 weekly care hours, and the spine fills `care_hours` from the FRS adult HOURTOT band with `would_claim_carers_allowance` following reported receipt. Paid-UC carer-element units 425,757 → 642,476 (+51 %); projection 899k against 1,078,514 (−17 %). Carer's Allowance spend stays on reported claimants once the take-up flag is re-derived after the SPI fill. The register row is retired.
- Childcare element: `would_claim_uc_childcare` is drawn per family type (single 0.441, couple 0.2682) so the expected paid-UC count equals the published calendar-2025 means at the Q50f weights; on the twin the realized count is 105,903 against 353,470 at rate one, below the expectation because SPI-synthetic units copy their donor's flag on a base of 378 and 201 records; projection 136.5k against 178,667 (−24 %). The register row is retired with that caveat recorded.
