...
The 45-node US combined completion-host test stopped at its 1,800-second CPU
limit without a completion receipt. Its 49- and 51-node status/role variants
remain on hold while we investigate repeated verification costs. The child property-income component has passed all 58
pure tests and all 14 actual-owner cases on invented inputs. The local host
repair now also passes all 50 small custody tests, including private-support
mutation and permanent-revocation checks. These tests establish component and
boundary behavior; the combined host and a native release file remain unverified.

The latest native property/tax attempt reached its CPU limit without a final
acceptance receipt. Paired measurements did not justify adopting either the
scanner or digest candidate: each slowed relevant workload classes. The
existing implementations remain. Repeated whole-frame and source verification
are a plausible bottleneck from source inspection. A small experiment using
the actual functions passed seven correctness controls and measured a median
18.36 ms per frame-identity call on 2,048 invented people. Actual call counts
and whole-build attribution remain unmeasured. No whole-build speedup, native
rerun or larger resource budget follows from these measurements.
Native PUF qualification still requires completion of applicable unknown inputs.
Adult routes outside the child model need aggregate source-incidence counts
before choosing whether another producer is necessary. Retirement
candidate accounting has passed 80 controls but does not yet supply fiscal
inputs. The
[dated evidence](../experiments/us-launch-consolidation-20260913.md#14-september-completion-checks)
records the accepted checks and remaining work.
