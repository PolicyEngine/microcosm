...
## Integration note (native integration line, 2026-09-23)

This note was written for `native-retention-seal` (#950). The native
integration line (`native-integration-20260923`) merges it into a tree that
already carried a second, opt-in answer to the same retention problem:
compact witness retention (`ee7049379`, `experiments/native-completion-retention/implementation.md`),
selected by `_population_retention="compact"`. The merge keeps both and
separates them by profile. Line numbers above are #950's and do not apply to
the merged file.

- **`_population_retention="all"` (the default) is this note's seal.** Every
  node is sealed on arrival with `replayed_population_seal`; only the declared
  consumers are retained, detached with `_observer_snapshot`; the executor is
  called with `_population_observer_detach=False`; replay comparisons use
  `same_replayed_population_seals`; `node_populations` carries the seal record
  for every dropped node.
- **`_population_retention="compact"` is unchanged.** It witnesses every node
  with `_survey_population_witness.population_witness`, retains its own
  compact roster, keeps the executor's detachment
  (`_population_observer_detach=True`) and compares with the witness or with
  `same_replayed_population` exactly as before.
- **The declared-consumer roster is the compact roster.** Both profiles take it
  from `_compact_retained_roster`: `financial.ATTACH_NODE`, the development
  attach node when source-qualified development inputs are enabled (the
  integration line makes it `final_node`, so it is an identity consumer #950
  did not have), the property attach and the tax gate.
- **`_retain_every_node_population`** is accepted by `_run_survey_financial`
  and by both public wrappers (`run_atomic_survey_financial`,
  `run_pre_geography_survey_financial`). The recursive completion base call
  sets it exactly when the profile is `"all"`; under `"compact"` the completion
  host reads witnesses instead of the base roster, so the flag is refused
  there under the existing `RETAIN_EVERY_NODE_POPULATION_FLAG` code.
- **Naming.** The section above explains why #950 avoided the name
  `population_retention`. The integration line's `_population_retention`
  keyword is `ee7049379`'s observer-retention profile, not the dropped #893
  keyword that made manifest views lazy.

No run on actual data has measured either profile's memory effect at scale.
