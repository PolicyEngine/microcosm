...
It carries two further corrections the 1/15 measurement harness did not: its
``sample.fraction`` and its ``scope`` sentence report 1/15, where that harness
left both as the tenth's 1/10 while running ``Fraction(1, 15)``. Neither field
reaches any code path; both are receipt prose.

Finally it carries the 1/10 refusal diagnostic's committed ``sys.monitoring``
RAISE block verbatim, inserted at the same point in the file, so the cause the
three nested catch-alls discard is observed rather than inferred.
...
# DIAGNOSTIC ONLY, and deliberately NOT a source change: the refusal path wraps
# its cause in three nested catch-alls that each `raise ... from None`, and the
# modules that hold two of them are sha256-pinned by
# ``acs_native_coverage_binding._ACCEPTED``, so editing them refuses with
# UNREVIEWED_PREPARATION before anything runs. ``sys.monitoring`` observes the
# raise without touching a byte of the measured tree.
import atexit  # noqa: E402

_RAISES = []
_TOOL = 5
_WRAPPERS = {
    "SurveyPopulationPreparationError",
    "ACSNativeCoverageBindingError",
    "ACSCoverageAuthenticationError",
    "ACSSourceCatalogueError",
    "AsecSourceCatalogueError",
}


def _on_raise(code, offset, exception):
    name = type(exception).__name__
    if name in ("StopIteration", "GeneratorExit", "KeyError", "AttributeError"):
        return
    _RAISES.append(
        {
            "type": type(exception).__module__ + "." + name,
            "message": str(exception)[:400],
            "file": code.co_filename.rsplit("/", 2)[-1],
            "function": code.co_qualname,
            "wrapper": name in _WRAPPERS,
        }
    )
    if len(_RAISES) > 400:
        del _RAISES[:200]
...
sys.monitoring.use_tool_id(_TOOL, "retention-diagnostic")
sys.monitoring.register_callback(_TOOL, sys.monitoring.events.RAISE, _on_raise)
sys.monitoring.set_events(_TOOL, sys.monitoring.events.RAISE)
atexit.register(
    lambda: (PROBE / "raises.json").write_text(
        json.dumps({"count": len(_RAISES), "tail": _RAISES[-120:]}, indent=1)
    )
)
