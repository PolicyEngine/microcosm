...
| Diff | **28 files** against the base branch at `3b63b8504`, the head the third fix pass pushed, in three parts: `packages/*/src` **12 files, +1,750 / −818**, which no edit to this report can move; `packages/*/tests` **6 files, +2,304**; docs, journal, changelog fragment, `.gitignore` and `experiments/` **10 files, +2,497 / −1**. The whole-branch total is +6,551 / −819 there. The third part and the total rise with every further commit to this file, which is why the parts are given as well and why each is dated to a sha. Two earlier readings are kept as what they were at their heads: **27 files**, src +1,683 / −818, tests **5 files, +1,799**, docs **10 files, +2,299 / −1**, total +5,781 / −819 at `db7a3a0ba`; and 26 files, tests **4 files, +1,780**, docs **10 files, +2,184 / −1**, total +5,647 / −819 at `baec266d2`. The file count moved 26 → 27 when the second pass touched `test_us_graph_atomic_survey_financial.py`, an existing file this branch had not touched, and 27 → 28 when the third pass added `packages/microcosm-graph/tests/test_graph_verification_epoch.py`. Two-dot and three-dot agree, because the branch was rebased onto the base tip `363a9033b` on 2026-09-16 and no base drift is left to inflate either; the merge-base is `363a9033b` and both forms report the same 28 files, +6,551 / −819. The earliest figure this table carried, 27 files / +4,399 / −849, was a two-dot diff taken while the branch was one commit behind, so its −30 in `docs/us-uk-release-path.md` was the base's own commit and not a lane change; it is superseded rather than corrected in place, because the tree has moved too. |
