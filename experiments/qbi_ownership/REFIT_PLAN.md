...
1. **Transfer-first incidence failures, plus the UBIA QED failure.** One QRF
   ...
   `packages/microcosm-build/src/microcosm/build/us_runtime/stacked_spine.py:8728-8772,8832-8852`). Drawn carrier incidence tracks
   ...
   BDC 0.0030 vs 0.0091), while the same chained models keep the
   ...
   availability-pattern routing, or chained conditioning causes it (six
   base predictors plus optional pattern predictors;
...
2. **The transfer does not condition on the exact coupled exposure bases.**
   ...
   (`packages/microcosm-build/src/microcosm/build/us_runtime/qbi_inputs.py:1342-1359`) then kill 71–76% of drawn carriers on both
   channels. The transfer feature surface includes non-qualified dividends
...
1. **Couple the amount draws to their exposure bases (defect 2).** Add the
   ...
   — or restructure batch_4 so BDC/REIT amounts are drawn conditional on a
   positive base — so drawn carriers land where the caps cannot destroy
   them. Introduce that QBI-specific surface at the family-fit/feature-surface
...
2. **Instrument the host-owned 1% diagnostic before selecting a gate fix
   ...
   features (`packages/microcosm-fit/src/microcosm/fit/qrf.py:1333-1442,1448-1497`).
...
4. **Rerun whole-pool reconciliation and the coupled identities after any
   ...
   zero at terminal, and the reconciliation change receipt must show the
   caps no longer destroying drawn carrier mass (defect 2's verification).
...
- **The 1% diagnostic and demonstration build** of actions 1–3. Start from
  ...
  memory envelope" (`tools/build_us_multispine_pool.py:18-19`) because PUF
