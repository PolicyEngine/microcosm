...
## R15 — Erratum: the ONS land rows were mis-measured by per-block engine resolution; two A16 exclusions withdrawn (2026-09-03)

Found while answering María's question about the land "spine defects". Comparing the initial national estimates of the 15-block run (R10's G at 1500 epochs) with the single-block run (R11), **3 of 364 national rows differ**:

| row | target | single block | 15 blocks | blocks / single |
|---|---|---|---|---|
| `ons.land.corporate_land_value` | £2.10tn | £2.04tn (−2.8 %) | £30.7tn | **15.000** |
| `ons.land.land_value` | £7.12tn | £6.47tn (−9.1 %) | £35.1tn | 5.42 |
| `slc.repayments.england_total_higher_education` | £5.02bn | £6.18bn (+23 %) | £6.07bn (+21 %) | 0.983 |

Mechanism: policyengine-uk's `corporate_land_value` distributes the parameter `aggregate_corporate_land_value` (£2.06tn at 2021, the ONS balance sheet's non-financial-corporation land) to households by their share of **total weighted `corporate_sector_wealth`**; `land_value` adds it to `household_land_value`. Under `--engine-blocks K` the engine runs per clone block, so "total" is the block's total and every block reproduces the whole aggregate: K× for the corporate row, (household + K × corporate) / (household + corporate) for the total. Every run in R6–R10 used 10 or 15 blocks, so the land rows there were inflated 10× / 15× (the 8.7× and 2.5× "misses" were 10 × 0.97 − 1 and the mixed ratio). The SLC repayments row moves 1.7 % for the same class of reason (a population-normalised term) and is noted, not acted on. **No other national row is block-sensitive** (361 of 364 identical), so the A2/A3/K/epoch conclusions stand; the two land rows sat at the loss cap in those runs and contributed at most 0.018 of the 0.32 initial loss and ~0.003 of the final loss under `grain_equal`, and 1/20,480 each under `uniform`.
