...
class UKGraphCalibrationConfig:
    """Solve and sizing settings, each a parameter of the node that uses it.

    ``baseline_pi_floor`` (#921) trims the refit's Horvitz–Thompson baseline
    (see :func:`~microcosm.build.uk_runtime.dataset_size.refit_uk_dataset_size`);
    it is a parameter of the size-refit node and of the rotated holdout that
    resizes with it, never of the search or the draw, so a resumed selection
    may refit under a different floor. ``0.0`` is the untrimmed default.
    """
