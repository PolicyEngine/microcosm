"""TRACE Transparent Research Object (TRO) emission for microcosm builds.

Emits JSON-LD that conforms to the TRACE TROv vocabulary
(https://w3id.org/trace/trov/0.1#) for a microcosm dataset build. A build TRO
...
so the agreement is auditable). microcosm-specific metadata lives under the
``pop:`` namespace so generated TROs can still be validated against TROv SHACL
shapes when tooling is available.

CLI::

    python -m microcosm.build.trace <release_manifest.json> [--out PATH] \\
        [--self-url URL]

reads a release manifest (the shape of
``packages/microcosm-data/build/us/release_manifest.json``) and writes the TRO
to ``--out`` or stdout.
"""
...
#: The microcosm-specific extension namespace. Mirrors the role of ``pe:`` in
#: policyengine.py (which uses ``https://policyengine.org/trace/0.1#``);
#: microcosm gets its own so the two never collide.
POPULACE_TRACE_NAMESPACE = "https://microcosm.institute/trace/0.1#"
...
def _microcosm_build_version() -> str | None:
    """The installed microcosm-build package version, for ``trov:createdWith``.
    ...
    try:
        ...
        return None
    try:
        return version("microcosm-build")
    except PackageNotFoundError:
        try:
            from microcosm.build import __version__
...
def _research_system() -> dict[str, Any]:
    return {
        ...
        "schema:name": "microcosm build pipeline",
        "rdfs:comment": (
            "The microcosm dataset build: a typed stage plan over primary "
...
def build_build_tro(
    *,
    ...
    """Build a TRACE TRO for a microcosm dataset build.
    ...
    Args:
        ...
        builder: Who/what ran the build (``"microcosm"``).
        ...
        stage_records: A sequence of ``StageRecord`` manifest dicts (NOT
            ...
            :mod:`microcosm.build.plan`), or ``None``.
    ...
    node: dict[str, Any] = {
        ...
        "schema:name": f"microcosm build TRO ({build_id})",
        "schema:description": (
            "TRACE TRO for a microcosm dataset build. The composition pins the "
        ...
        "trov:createdWith": {
            ...
            "schema:name": "microcosm-build",
            "schema:softwareVersion": _microcosm_build_version(),
        },
...
def _build_performance(
    *,
    ...
    performance: dict[str, Any] = {
        ...
        "rdfs:comment": f"microcosm build {build_id}.",
...
#: ``microcosm.data.registry``). The release manifest does not record its
...
def tro_from_release_manifest(
    ...
    *,
    ...
    """Convert a microcosm release manifest into a TRACE TRO.
    ...
    if isinstance(build_info, Mapping):
        ...
        builder = str(build_info.get("builder", manifest.get("builder", "microcosm")))
        ...
        builder = str(manifest.get("builder", "microcosm"))
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        prog="microcosm.build.trace",
        description=(
            "Emit a TRACE Transparent Research Object (TRO) from a microcosm "
            "release manifest."
        ),
    )
    parser.add_argument(
