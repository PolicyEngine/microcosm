...
from .compiler_ir import CompiledSpecIR
...
def _us_domains(
    spec: ResolvedSpec | CompiledSpecIR,
) -> dict[str, dict[str, object]]:
    country = (
        spec.country if isinstance(spec, ResolvedSpec) else spec.spec_binding.country
    )
    if country != "us":
        raise SpecValidationError(
            "stacked checkpoint semantics require the normalized US bundle"
        )
    if isinstance(spec, CompiledSpecIR):
        # This path is used only while issuing the narrow frozen generation-0
        # compatibility projection.  Runtime capabilities never retain the
        # normalized IR or expose its operational/documentation fields.
        normalized = spec.resources_wire()
        ...
        for kind in (
            ...
            result[kind] = deepcopy(
                dict(
                    _mapping(
                        normalized.get(kind),
                        location=f"resources/{kind}",
                    )
                )
            )
        return result
...
def project_stacked_authority_receipt(
    spec: ResolvedSpec | CompiledSpecIR,
) -> dict[str, object]:
...
def _build_model_seed(spec: ResolvedSpec | CompiledSpecIR) -> int:
    if isinstance(spec, ResolvedSpec):
        ...
        defaults = {
            ...
            for site in spec.seed_stream_map.sites
            for contract in [
                _mapping(
                    thaw_json(site.contract),
                    location=f"seed_stream_map/sites/{site.id}/contract",
                )
            ]
            if contract.get("value_source") == "run_request.build_model_seed"
            and contract.get("default") is not None
        }
    if len(defaults) != 1:
        raise SpecValidationError(
