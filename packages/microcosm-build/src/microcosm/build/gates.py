...
- :func:`export_surface_gate` and :func:`target_surface_gate` — replacement
  ...
  calibration targets, e.g. UK Microcosm against eFRS.
- :func:`input_mass_parity_gate` — persisted input columns must carry their
  ...
  its own target surface perfectly (microcosm issue #278).
...
#: (``microcosm.fit.NO_WEIGHTS``). Kept as a literal here rather than imported so
#: the build shard's gate does not pull in ``microcosm.fit`` (and its
...
#: :class:`~microcosm.frame.WeightKind` provenance (``microcosm.fit.EXPLICIT_WEIGHTS``).
...
#: typed :class:`~microcosm.frame.WeightKind` values (``design`` / ``importance``
...
#: ``microcosm.fit.resolved_weight_kind`` produces. Only ``none`` is treated as
...
class FitWeightRecord:
    ...
    The microcosm-fit operator is weighted by construction: a Frame fit defaults
    to typed design weights and ``weights="none"`` is the only unweighted path
    (``microcosm.fit.model``). :func:`weights_audit_gate` lifts that API guarantee
    ...
    Attributes:
        ...
        weight_kind: The kind the fit *resolved* to, from
            ...
            :class:`~microcosm.frame.WeightKind` value (``"design"`` /
            ``"importance"`` / ``"calibrated"``) or ``"none"`` for a
            deliberately unweighted fit. A :class:`~microcosm.frame.WeightKind`
    ...
    def __init__(self, fit_name: str, weight_kind: str | WeightKind) -> None:
        ...
        if weight_kind not in RESOLVED_WEIGHT_KINDS:
            raise ValueError(
                ...
                "unauditable — record the kind microcosm.fit resolved."
            )
...
def export_surface_gate(
    ...
    *,
    ...
    country has a known incumbent-compatible artifact, such as UK Microcosm
...
def input_mass_parity_gate(
    ...
    *,
    ...
    reform touching that base silently scores ~$0 (microcosm issue #278,
...
def source_coverage_gate(
    ...
    *,
    ...
    ``SourceCoverageEntry`` dataclass from ``microcosm.build.us_runtime.source_coverage``
...
def source_stage_input_coverage_gate(
    ...
    *,
    ...
    microcosm issues #253 and #252). This gate makes that a build failure.
...
def input_column_coverage_gate(
    ...
    *,
    ...
    This is the release-blocking column-coverage contract of microcosm #368: a
    ...
    credits, IRA/HSA — microcosm #340/#356/#361/#278) is *absent from the export
...
def aggregate_admin_gate(
    ...
    *,
    ...
    registry :class:`~microcosm.calibrate.registry.TargetSpec` (value +
...
def weights_audit_gate(
    ...
    *,
    ...
    microcosm-fit already makes an unweighted fit impossible to request without
    writing ``weights="none"`` and meaning it (``microcosm.fit.model``). This gate
    ...
    for fit_name in sorted(resolved_kinds):
        ...
        failures.append(
            ...
            "allowed-unweighted entry. A microcosm fit is weighted by "
...
class TargetFitRequirement:
    ...
    enumerated in advance. microcosm#462 is what slips through: the live Build M
...
def target_fit_gate(
    ...
    *,
    ...
    The microcosm#462 catcher, generalized: the Build M live release recorded a
...
def tail_concentration_gate(
    ...
    *,
    ...
    The microcosm#462 signature: the Build M ``puf_tax_detail`` weighted-QRF
    ...
    for column in sorted(column_values):
        ...
        if share > max_top_share:
            failures.append(
                ...
                "a sparse imputation (the microcosm#462 "
