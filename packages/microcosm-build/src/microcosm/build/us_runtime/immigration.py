...
#: PEINUSYR code → retained integer arrival-year approximation. In the official
#: 2025 dictionary code 28 is 2022-2024, while cpsmar25.pdf prints 2022-2025;
#: 2023 is the midpoint/floor-midpoint convention for either interval, not an
#: exact entry year. Both enumerate 1..28 (no 29), despite stale 0:26 headers.
#: See docs/us-immigration-observation-years.md for both sources and limitations.
...
_EVIDENCE_FEATURE_VERSION = "census-entry-approximation-no-arrival-zero@2"
_NO_ARRIVAL_EVIDENCE_FEATURE_VALUE = 0


def us_immigration_evidence_feature_contract() -> dict[str, object]:
    """Describe value semantics bound into the transfer checkpoint identity."""
    return {
        "version": _EVIDENCE_FEATURE_VERSION,
        "no_arrival_evidence_value": _NO_ARRIVAL_EVIDENCE_FEATURE_VALUE,
        "no_arrival_evidence": "ASEC PEINUSYR=0 or ACS YOEP nonfinite",
        "asec_arrival_year_midpoints": {
            str(code): year for code, year in sorted(_ARRIVAL_YEAR_MIDPOINTS.items())
        },
        "asec_top_code_approximation": "28 -> 2023; unresolved upper endpoint 2024/2025",
        "acs_arrival_year": "retain YOEP literal; 1938 and 1939 are coarsened codes",
        "origin_codes": "raw survey codes; only reviewed named-country selectors correspond",
    }
...
#: PEINUSYR codes for 2020+ arrivals. The reviewed bins distinguish 2020-2021
#: (27) from 2022 onward (28; disputed 2024/2025 upper endpoint in 2025 docs).
#: Program-specific rules below do not treat those
...
#: Named-country selectors checked against Census cpsmar25.pdf Appendix J and
#: the 2024 ACS PUMS Place of Birth code list. Their complete code domains are
#: not interchangeable. Cuba (327) and Haiti (332) stay out of every draw:
...
#: Burma, Venezuela lead). The code list distinguishes 412 Congo from
#: 459 Zaire (DR Congo); these are separate named-country selectors.
...
_TPS_ORIGIN_CODES: Mapping[str, tuple[tuple[int, ...], int]] = {
    ...
    "nicaragua": ((315,), 16),
    "nepal": ((229,), 24),
    # Burma, Syria, Yemen, Lebanon, Cameroon, Ethiopia, Somalia, Sudan only.
    # No separately qualified South Sudan code is inferred here.
    "other_designated": ((205, 239, 248, 224, 407, 416, 448, 451), 28),
}

#: Modern ACS YOEP values preserve a reported year, unlike ASEC arrival bins;
#: 1938 means 1938 or earlier and 1939 means 1939-1944. Use the modern-year
#: information for approximate continuous-residence compatibility rather than
...
_TPS_ACS_MAX_ARRIVAL_YEAR_BY_BIRTH: Mapping[int, int] = {
    ...
    416: 2024,  # Ethiopia
    448: 2024,  # Somalia
    451: 2023,  # Sudan only; ACS South Sudan is inseparable within code 464.
}
...
def _observation_years(
    person: pd.DataFrame,
    *,
    time_period: int,
    observation_year_column: str | None,
) -> np.ndarray:
    """Read explicit row years without inferring authority or filling missing cells.

    The source owner must bind years to its survey. This helper validates the
    numeric calendar-year surface only. Omission retains legacy scalar behavior.
    """
    if observation_year_column is None:
        if isinstance(time_period, bool) or int(time_period) != time_period:
            raise ValueError("US immigration time_period must be an integer.")
        return np.full(len(person), int(time_period), dtype=np.int64)
    if not isinstance(observation_year_column, str) or not observation_year_column:
        raise ValueError(
            "US immigration observation_year_column must be a nonempty string."
        )
    if observation_year_column not in person.columns:
        raise ValueError("US immigration observation-year column is missing.")
    values = person[observation_year_column].to_numpy(dtype=object)
    if values.shape != (len(person),) or any(
        ...
        raise ValueError(
            "US immigration observation years require one finite integral calendar year per person; no missing, boolean, or coerced values."
        )
...
def _source_aware_immigration_profile(
    ...
    *,
    ...
    present. Only the reviewed named-country selectors share code values;
    complete POBP/PENATVTY domains differ and raw codes are not a lossless
    crosswalk. ACS YOEP retains the reported literal (1938/1939 are coarsened),
    while ASEC uses the explicit integer interval approximation. This rule
    helper does not qualify source domains or establish statutory eligibility.
    """

    observation_years = _observation_years(
        person, time_period=time_period, observation_year_column=observation_year_column
    )
    ...
    if invalid_asec_arrival.any():
        ...
        raise SourceRuntimeError(
            "PEINUSYR carries value(s) outside the reviewed ASEC code domain "
    ...
    arrival_year = observation_years.copy()
    ...
    invalid_acs_arrival = acs_foreign_born & (
        ...
        | (acs_arrival < 1900)
        | (acs_arrival > observation_years)
    )
    if invalid_acs_arrival.any():
        raise SourceRuntimeError(
            f"ACS YOEP must be an integral 1900..{'time_period' if observation_year_column is None else 'observation_year'} year for every "
    ...
    age_at_entry = np.maximum(0, age - (observation_years - arrival_year))
...
def us_immigration_evidence_features(
    ...
    *,
    ...
    """Return finite immigration predictors with a year-independent NIU value."""

    person = frame.table(frame.schema.person_entity)
    profile = _source_aware_immigration_profile(
        person, time_period=time_period, observation_year_column=observation_year_column
    )
    observed_arrival = np.where(
        profile.source_is_acs,
        np.isfinite(_numeric_evidence_column(person, "YOEP")),
        profile.arrival_code != 0,
    )
    return pd.DataFrame(
        {
            "is_us_citizen": profile.is_citizen.astype(np.float64),
            "birth_country_code": profile.birth_country.astype(np.float64),
            "arrival_year": np.where(
                observed_arrival,
                profile.arrival_year,
                _NO_ARRIVAL_EVIDENCE_FEATURE_VALUE,
            ).astype(np.float64),
        },
        index=person.index,
    )
...
def _arrival_profile(
    ...
    *,
    ...
    years_in_us = observation_years - arrival_year
...
def us_immigration_humanitarian_transfer_selection_masks(
    ...
    *,
    ...
    Supplying the descriptive reconciliation receipt additionally refuses a
    changed selection weight vector, mutable-row mask or seed. It is not source
    authority; callers must still retain the same evidence and controls.
    """
    ...
    if reconciliation_receipt is not None and (
        reconciliation_receipt.get("kind")
        != "deterministic_humanitarian_residual_target"
        or reconciliation_receipt.get("seed") != int(seed)
        or reconciliation_receipt.get("selection_inputs")
        != _transfer_selection_inputs(weights, mutable)
    ):
        raise ValueError(
            "Humanitarian transfer selection replay receipt disagrees with "
            "resolved weights, mutable_rows or seed."
        )
...
def _transfer_selection_inputs(weights, mutable) -> dict[str, object]:
    """Ordered numerical selection inputs, after shape/domain validation."""
    return {
        "version": 1,
        "rows": len(weights),
        "resolved_person_weights_sha256": hashlib.sha256(
            np.ascontiguousarray(weights, dtype="<f8").tobytes()
        ).hexdigest(),
        "mutable_rows_sha256": hashlib.sha256(
            np.ascontiguousarray(mutable, dtype=np.uint8).tobytes()
        ).hexdigest(),
    }
...
def reconcile_us_immigration_humanitarian_transfer(
    ...
    *,
    ...
    controls = us_immigration_controls() if controls is None else controls
    profile = _source_aware_immigration_profile(
        result, time_period=time_period, observation_year_column=observation_year_column
    )
    ...
    receipt: dict[str, object] = {
        ...
        "seed": int(seed),
        "time_period": int(time_period) if observation_year_column is None else None,
        "selection_inputs": _transfer_selection_inputs(weights, mutable),
    ...
    if observation_year_column is not None:
        ...
        receipt["observation_years"] = {
            "mode": "per_row",
            "column": observation_year_column,
            "row_counts": {
                str(int(y)): int(n) for y, n in zip(values, counts, strict=True)
            },
        }
...
def _assign_ssn_card_codes(
    ...
    *,
    ...
    _, arrival_year, age_at_entry = _arrival_profile(
        person, time_period=time_period, observation_year_column=observation_year_column
    )
...
def with_us_immigration_inputs(
    ...
    *,
    ...
    Args:
        ...
        observation_year_column: Explicit numeric person column for source
            observation years. When provided it governs both arrival helpers;
            omission retains scalar ``time_period`` behavior. This argument
            validates values but does not authenticate their source.
...
def us_immigration_composition_gate(
    ...
    *,
    ...
    try:
        evidence = _source_aware_immigration_profile(
            person,
            time_period=time_period,
            observation_year_column=observation_year_column,
        )
        if observation_year_column is not None:
            details["observation_years"] = {
                "mode": "per_row",
                "column": observation_year_column,
            }
