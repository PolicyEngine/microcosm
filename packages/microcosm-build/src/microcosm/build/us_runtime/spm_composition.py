...
SPM_COMPOSITION_REMEDY = (
    "Remedy: supply the measured SPM independence role, "
    ...
    "role rule (microcosm.build.us_runtime.spm_role_source.SPM_ROLE_RULE). "
    "Carrying only is_household_head / is_household_spouse lets the engine's "
    "fallback formula classify a 15-to-17-year-old head or spouse, but it is "
    "not a release remedy: a US release requires the source role itself "
    "(release input coverage, the export's required person source columns and "
    "the SPM independence role signal gate), and the fallback measures "
    "household headship, not SPM-unit independence. "
