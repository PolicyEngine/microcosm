...
# Version 2 (microcosm#469): count-matching-era fields (reachable_goal) are
...
# Version 3 (microcosm#507/#508): the top level documents the prior weight
...
#: (microcosm#507 sol review finding 1).
...
#: (microcosm#453 / PR #509) — never as a side effect.
...
#: (Build M, microcosm#507 table); 5% leaves the solver headroom while any
#: collapse-class regression (Build N's 65+ shipped 59% under target) fails
#: loudly. One uniform constant — never a per-target knob (microcosm#492).
...
class SSITakeUpAgeTarget:
    ...
    ``targets`` read from the calibration registry (microcosm#469/#470), never
    as module constants.
    """
...
class SSITakeUpPriorBasis:
    ...
    ``us_ssi_take_up.json`` diagnostics, the microcosm#508 remedy: thresholds
...
def us_ssi_take_up_stage_spec() -> SourceStageSpec:
    ...
    if "target_values" in target_artifacts[0]:
        raise ValueError(
            ...
            "as ledger-fed calibration registry targets (microcosm#469/#470)."
        )
    return spec
...
def ssi_take_up_prior_basis_from_artifact(
    ...
    *,
    ...
    This is the microcosm#507/#508 remedy path: the payload is a prior
    ...
    band values — microcosm#508's "one coherent system"), and every enforced
    ...
    if schema_version == _DIAGNOSTICS_SCHEMA_VERSION:
        ...
        if artifact_prior_kind == US_SSI_TAKE_UP_PRIOR_BASIS_RELEASE_ARTIFACT:
            ...
            raise ValueError(
                ...
                "deleted microcosm#463-class loop — microcosm#508 permits "
    ...
    for key, row in rows.items():
        ...
        if key in US_SSI_TAKE_UP_ENFORCED_BAND_KEYS and capacity <= target:
            ...
            # EXPECTED to saturate until microcosm#453/#509 lands.
            raise ValueError(
...
def _assign_sources(
    ...
    *,
    ...
    for target_definition in US_SSI_TAKE_UP_AGE_TARGETS:
        ...
        # reporters anchored unconditionally (microcosm#469). No count
...
def us_ssi_take_up_gate(
    ...
    *,
    ...
    for key, target in expected_targets.items():
        ...
        # measurement (microcosm#507/#508). The recomputed prior is defined on
...
def us_ssi_take_up_delivery_gate(
    ...
    *,
    ...
    microcosm#507 shipped a certified default whose 65+ SSI baseline ran 59%
    ...
    (microcosm#477's cutover). This gate makes an enforced band miss a
    ...
    per-target knob (microcosm#492). The under-18 band stays fenced pending
    microcosm#453/#509 and is reported in the details, never enforced.
    ...
    on the dense full-pool arm, microcosm#508 delivered-weight recomputes
    ...
    (microcosm#566/#567) — so its adult bands ship in the scorecard as
        ...
        if key in US_SSI_TAKE_UP_ENFORCED_BAND_KEYS and fence_text is None:
            ...
            if abs(selected - target) > tolerance * target + 1e-6:
                failures.append(
                    ...
                    "delivered weights (microcosm#507/#508)."
                )
        else:
            fenced_rows.append(
                {
                    **summary,
                    "fence": (
                        ...
                        if fence_text is not None
                            ...
                            "qualifying-disability stage (microcosm#453 / PR "
