...
(:mod:`microcosm.build.us_runtime.release_input_coverage`). Where that module
...
pipeline calibrated to is silently absent from the microcosm target registry.
...
#:   the microcosm#508 owner adjudication (2026-07-23) the three by-age band
...
def _is_target_period_token(value: str) -> bool:
    ...
    lockstep with
    :func:`microcosm.build.ledger_targets._is_period_token` so the family id is
...
def us_release_target_parity_gate(
    ...
    *,
    ...
    Args:
        registry: The compiled :class:`~microcosm.calibrate.TargetRegistry`.
    ...
    for family in sorted(reviewed & present):
        failures.append(
            ...
            "(the manifest cannot rot — PolicyEngine/microcosm#286/#337)."
        )
