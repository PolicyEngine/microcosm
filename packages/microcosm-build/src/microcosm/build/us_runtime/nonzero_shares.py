...
:func:`microcosm.build.gates.parity_gate` compares, for every layer the incumbent
...
:func:`microcosm.build.plan._nonzero_share` so the pinned incumbent reference and
a live candidate frame are measured the same way.

Unlike :func:`microcosm.build.us_runtime.input_mass.us_input_mass_totals`, which
...
def nonzero_share(column: pd.Series) -> float:
    ...
    columns (:func:`microcosm.build.plan._nonzero_share`), kept in lockstep so
