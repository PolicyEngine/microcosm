...
whose selection descended from an uncommitted run-dir artifact (microcosm#328).
...
one already used for seeded draws in :mod:`microcosm.build.us_runtime.take_up`:
...
def load_selection_source_from_h5(
    ...
    *,
    ...
    ``source`` is a microcosm US frame (e.g. the certified live default loaded via
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Distill a microcosm US H5's record set into a committed "
            "selection-source manifest (microcosm#328). The manifest names the "
