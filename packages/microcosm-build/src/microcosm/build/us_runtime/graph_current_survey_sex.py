"""Visible source-sex binding once per original person, then exact clone copy.
...
from . import current_survey_sex_source as source
from .graph_survey_population import SOURCE_NAME

require = source.require
SOURCE_NODE = "survey_sex.source"
RAW_NODE = "survey_sex.raw_columns"
BIND_NODE = "survey_sex.bind_is_female"
ATTACH_NODE = "survey_sex.attach"
PROJECTION_TYPE = ArtifactType("microcosm.us.current_survey_sex_projection", 1)
COLUMNS_TYPE = ArtifactType("microcosm.us.current_survey_sex_columns", 1)
ATTACHMENT_TYPE = ArtifactType("microcosm.us.current_survey_sex_attachment", 1)
...
def _source_edge():
    return ArtifactInput("sex_projection", SOURCE_NODE, "projection", PROJECTION_TYPE)


def _columns_edge(node_id):
    return ArtifactInput(
        "sex_raw" if node_id == RAW_NODE else "sex_canonical",
        node_id,
        "columns",
        COLUMNS_TYPE,
    )
...
def sex_nodes(qualified, receiving, *, receiving_version, after):
    ...
    params = {
        ...
        "mapping": "1->false, 2->true, unresolved->null; ASEC requires named AXSEX",
        "acs_allocation_provenance": "unresolved",
    }
    create = Node(
        SOURCE_NODE,
        "us.survey_sex.source@1",
        structural=StructuralDelta.CREATE,
        sources=(SOURCE_NAME,),
        outputs=(Owned("person", "sex_native_person_id", "int64"),),
        params=params,
        artifact_inputs=(after,),
        artifact_outputs=(ArtifactOutput("projection", PROJECTION_TYPE),),
        description="Borrow authenticated original ACS/ASEC support and private sex/allocation observations.",
    )
    raw = Node(
        RAW_NODE,
        "us.survey_sex.raw@1",
        ...
        description="Expose source sex codes, ASEC allocation flags, observation year and unresolved provenance.",
    )
    bind = Node(
        BIND_NODE,
        "us.survey_sex.bind@1",
        population=SOURCE_NODE,
        ...
        artifact_outputs=(ArtifactOutput("columns", COLUMNS_TYPE),),
        description="Bind nullable is_female once per original person; unresolved source or ASEC allocation remains unknown.",
    )
    provenance = source.attachment.provenance
    attach = Node(
        ATTACH_NODE,
        "us.survey_sex.attach@1",
        ...
        description="Copy each qualified original's sex and provenance to its existing two clones, preserving unknowns and weights.",
    )
...
def sex_result(qualified, node, artifacts, people=None):
    ...
    if node.id == SOURCE_NODE:
        ...
        tables["person"]["sex_native_person_id"] = (
...
class _SexKernel(KernelBase):
    ...
    def implementation_hash(self):
        return source_hash(
            ...
            source.asec.household,
            source.asec.source_csv_builtin,
            source.original,
            source.attachment,
            source.source,
            source.physical,
...
def sex_kernels(
    ...
    return tuple(
        _SexKernel(qualified, nodes, require_current, require_context, ref)
