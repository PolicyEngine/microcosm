...
- education credits (``soi_baseline_levels`` row ``soi_education_credits``)
  ...
  produces that leaf and its five AOTC factual inputs (microcosm#253); and
...
from microcosm.build.gates import GateResult, source_stage_input_coverage_gate
...
def _source_stages_path() -> Path:
    return Path(str(files("microcosm.build.us").joinpath("source_stages.json")))
...
def _validation_leaf_engine() -> Any | None:
    ...
    Mirrors :func:`microcosm.build.us_runtime.puf_support._formula_owned_engine`:
    the adapter ships with microcosm-frame and imports ``policyengine_us`` lazily,
...
def assert_validation_leaf_registry_current(
    *,
    ...
    (cf. :func:`microcosm.build.us_runtime.puf_support.assert_formula_owned_blocklist_current`).
