...
did not ship at 100% participation; microcosm produces almost none of them, so
...
class TakeUpProgram:
    ...
    def is_seeded(self) -> bool:
        """Whether microcosm assigns this flag from a sourced rate."""
...
def _contract_path():
    return files("microcosm.build.us").joinpath("take_up_contract.json")
...
def seeded_take_up_programs() -> tuple[TakeUpProgram, ...]:
    """The programs microcosm seeds, in table order."""
    return load_take_up_contract().seeded()


def count_calibrated_take_up_programs() -> tuple[TakeUpProgram, ...]:
    """The programs microcosm assigns via anchored count-calibration."""
    return tuple(
        program
...
def assert_take_up_contract_current(
    ...
    *,
    ...
    - any asserted engine fact disagrees (e.g. policyengine-us gives a
      ...
      microcosm must stop seeding it).
    ...
    *,
    ...
    for program in resolved.programs:
        ...
        if cls == "model_simulated" and treatment != "model_simulated":
            # A model-simulated flag's only microcosm-native option besides
