...
``generator`` block say so. Fact keys live in a microcosm namespace
...
IMDB_BULK_SOURCE_LEG = FactSourceLeg(
    ...
    extraction_method=(
        "microcosm us_trade imdb_bulk ingest: Census monthly bulk IMDB "
        ...
        "microcosm-minted to the ledger consumer contract (not a "
...
IMDB_DISTRICT_SOURCE_LEG = FactSourceLeg(
    ...
    extraction_method=(
        ...
        "against them; microcosm-minted to the ledger consumer contract "
...
CENSUS_API_SOURCE_LEG = FactSourceLeg(
    ...
    extraction_method=(
        "microcosm us_trade census_imports ingest: Census International "
        "Trade API monthly HS10 chapter pulls, DET country detail summed "
        "to the {grain} grain and reconciled exactly against the "
        "publisher's '-' totals; microcosm-minted to the ledger consumer "
...
#: ``chapter_country`` is the P3 dashboard axis (microcosm#615) and carries
...
MEASURE_CATALOG: Mapping[str, Mapping[str, str]] = {
    ...
    "cal_dut_mo": {
        ...
        "concept_evidence": (
            ...
            "not error, and this series never gates (microcosm#615)."
        ),
    },
    "dut_val_mo": {
    ...
    *,
    ...
    for cell in stats.exact_cells():
        ...
        source = {
            ...
            "extraction_method": (
                "microcosm us_trade cbp_entry_stats ingest: archived public "
                "statistics page parsed from recorded bytes; microcosm-minted "
...
def _stamp_keys(row: dict[str, Any]) -> None:
    """Deterministic microcosm-namespace fact keys from row identity.
...
def default_generator_block(
    *, months: tuple[str, ...], now: datetime | None = None
    ...
    """Manifest ``generator`` block naming the microcosm producer."""
    moment = now or datetime.now(UTC)
    return {
        "producer": "microcosm.build.us_runtime.us_trade",
        "issue": "PolicyEngine/microcosm#615",
        "created_at": moment.isoformat(timespec="seconds"),
        "months": list(months),
        "note": (
            "Microcosm-minted ledger-contract feed for the import-entry unit "
