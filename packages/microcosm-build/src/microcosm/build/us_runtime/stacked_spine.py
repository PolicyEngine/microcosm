...
# v12 binds the union of the per-target terminal comparison roles and the
# post-transfer two-part calibration policy, which independently used v11 on
# their source branches. v10 bound the primary-PUF whole-pool output universe.
...
_EXPECTED_ORIGIN_BATTERY_ROLE_DECLARATIONS: Mapping[tuple[str, str, str], int] = (
    MappingProxyType(
        {
            (
                "person",
...
_EXPLICIT_ORIGIN_BATTERY_ROLE_DECLARATIONS = _EXPECTED_ORIGIN_BATTERY_ROLE_DECLARATIONS
...
def _validate_explicit_origin_battery_role_declarations(
    ...
    if malformed:
        raise RuntimeError(
            f"Battery role declarations contain malformed target keys: {malformed}."
        )
    ...
    *,
    ...
    for field_name, value in counts.items():
        if not isinstance(value, int) or isinstance(value, bool) or value < 0:
            raise ValueError(
                f"{boundary}: ACS transfer row-count has invalid {field_name}."
            )
    typed_counts = {field: int(value) for field, value in counts.items()}
    if typed_counts["authorized_null_rows"] != (
        typed_counts["imputed_rows"] + typed_counts["unmodeled_rows"]
    ):
        raise ValueError(
            f"{boundary}: ACS transfer row-count activation accounting "
            "equation changed."
        )
    if typed_counts["residual_null_rows"] != typed_counts["unmodeled_rows"]:
        raise ValueError(
            f"{boundary}: ACS transfer row-count residual-null accounting "
            "equation changed."
        )
    return typed_counts
...
def _validate_acs_transfer_origin_pattern_authority(
    origin: object,
    *,
    expected_entity: str,
    ...
    """Bind a QRF origin catalog to the canonical predictor authority."""
    ...
    if origin["channel"] != "qrf_transfer":
        return
    _required, optional_authority = _acs_pattern_predictor_authority(
        entity=expected_entity,
        family_targets=expected_family_targets,
    )
    catalog = origin["availability_patterns"]
    assert isinstance(catalog, list)
    observed_options: list[tuple[str, ...]] = []
    observed_codes: list[int] = []
    for position, pattern in enumerate(catalog):
        assert isinstance(pattern, Mapping)
        optional = pattern["observed_optional_predictors"]
        assert isinstance(optional, list)
        optional_tuple = tuple(optional)
        if optional_tuple != tuple(
            predictor for predictor in optional_authority if predictor in optional_tuple
        ):
            raise ValueError(
                f"{boundary} availability-pattern catalog entry {position} "
                "is outside canonical transfer predictor authority."
            )
        observed_options.append(optional_tuple)
        observed_codes.append(
            sum(1 << optional_authority.index(item) for item in optional_tuple)
        )
    if (
        len(set(observed_options)) != len(observed_options)
        or observed_codes != sorted(observed_codes)
        or len(set(observed_codes)) != len(observed_codes)
    ):
        raise ValueError(
            f"{boundary} availability-pattern catalog order or membership is "
            "outside canonical transfer authority."
        )
...
def _validate_acs_imputed_pattern_evidence(
    ...
    *,
    ...
    _validate_acs_transfer_origin_pattern_authority(
        origin,
        expected_entity=expected_entity,
        expected_family_targets=expected_family_targets,
        boundary=f"{boundary} origin",
    )
    assert isinstance(origin, Mapping)
    if origin["channel"] != "qrf_transfer":
        raise ValueError(
            f"{boundary}: ACS QRF pattern evidence requires QRF origin evidence."
        )
    catalog = origin["availability_patterns"]
    ...
    evidence_catalog = [
        {
            "name": pattern["name"],
            "observed_optional_predictors": pattern["observed_optional_predictors"],
        }
        for pattern in patterns
    ]
    if evidence_catalog != catalog:
        raise ValueError(
            f"{boundary}: ACS QRF pattern evidence catalog differs from its "
            "origin receipt."
        )
    for pattern in patterns:
        selected_regimes = {
            item["model_target"]: item["regime"] for item in pattern["target_regimes"]
        }
        pattern_name = pattern["name"]
        if selected_regimes.get(model_target) != regimes[pattern_name]:
            raise ValueError(
                f"{boundary}: ACS QRF selected regime differs from the "
                f"canonical origin regime for {model_target!r} in "
                f"{pattern_name!r}."
            )
...
def validate_stacked_gap_fill_receipt(
    ...
    *,
    ...
    """Reject an early receipt without complete origin and calibration proof."""
...
_STACKED_GAP_FILL_DETERMINISTIC_ORIGIN_BINDINGS: Mapping[tuple[str, str], str] = (
    MappingProxyType(
        {
            (
                "person",
...
def _validate_stacked_gap_fill_receipt(
    ...
    *,
    ...
    expected_policy_sha256 = str(authority.post_transfer_calibration["sha256"])
    early_specs = {
        spec.key: spec
        for spec in (
    ...
    for direction_name, direction in expected_directions.items():
        ...
        if "post_transfer_calibration" not in direction_receipt:
            raise ValueError(
                f"{boundary}: stacked gap-fill direction {direction_name!r} "
                "has stripped or misbound calibration summary evidence."
            )
        if set(direction_receipt) != expected_direction_fields:
            raise ValueError(
                f"{boundary}: stacked gap-fill direction {direction_name!r} "
                "receipt schema drifted."
            )
        ...
        expected_targets = {
            ...
            for key, context in direction_target_context.items()
        }
        targets = direction_receipt.get("targets")
        if not isinstance(targets, Mapping) or set(targets) != set(expected_targets):
            raise ValueError(
                f"{boundary}: stacked gap-fill direction {direction_name!r} "
                "target surface is non-canonical."
            )
        ...
        for label, (family, target) in expected_targets.items():
            ...
            if "qrf_pattern_evidence" in target_receipt:
                expected_keys.add("qrf_pattern_evidence")
            if "post_transfer_calibration" in target_receipt:
                expected_keys.add("post_transfer_calibration")
            ...
            _validate_acs_transfer_origin_pattern_authority(
                ...
                expected_family_targets=direction.target_families[entity][family],
                boundary=f"{boundary} target {label!r} origin",
            )
            assert isinstance(origin, Mapping)
        ...
        if set(targets) != direction_target_keys:
            raise ValueError(
                f"{boundary}: stacked gap-fill direction {direction_name!r} target "
                "surface is non-canonical."
            )
        expected_calibrations = (
            {
                key: early_specs[key]
                for key in direction_target_keys
                if key in early_specs
            }
            if production
            else {}
        )
        ...
        for key, target_receipt in targets.items():
            ...
            if spec is None:
                evidence_spec = early_specs.get(key)
                if "qrf_pattern_evidence" in target_receipt and (
                    production or evidence_spec is None
                ):
                    raise ValueError(
                ...
                if "qrf_pattern_evidence" in target_receipt:
                    ...
                    entity, family, target, family_targets = direction_target_context[
                        key
                    ]
    ...
    if production and validated_keys != set(early_specs):
        raise ValueError(
...
def _canonical_post_puf_producer_roles(
    *,
    entity: str,
    target: str,
) -> tuple[str, ...]:
    """Return the canonical ordered producer roles for one late target."""

    role_surfaces = (
        (
            "puf_clone",
            _CANONICAL_STACKED_AUTHORITY.post_puf_puf_producer_surface,
        ),
        (
            "asec_source",
            _CANONICAL_STACKED_AUTHORITY.post_puf_source_producer_surface,
        ),
    )
    roles = tuple(
        role
        for role, surface in role_surfaces
        if any(
            candidate_entity == entity and candidate_target == target
            for candidate_entity, families in surface.items()
            for targets in families.values()
    ...
    if not roles:
        raise RuntimeError(
            "Canonical post-PUF transfer target has no declared producer role: "
            f"{entity}/{target}."
        )
    return roles


def _validate_stacked_post_puf_ownership_fields(
    receipt: Mapping[str, object],
    *,
    boundary: str,
) -> None:
    """Require a transfer receipt to name its one canonical ownership route."""
    ...
    for receipt_field, expected_value in expected.items():
        observed = receipt.get(receipt_field)
        if (
            receipt_field == "donor_clone_index"
            and (isinstance(observed, bool) or not isinstance(observed, int))
        ) or observed != expected_value:
            raise ValueError(
                f"{boundary}: stacked post-PUF transfer ownership "
                f"field {receipt_field!r} is {observed!r}, expected "
                f"{expected_value!r}; "
                "production manifest emission is forbidden."
            )


def _validate_stacked_post_puf_aggregate_ownership(
    receipt: Mapping[str, object],
    *,
    boundary: str,
) -> None:
    """Require the exact aggregate schema and canonical ownership route."""
    ...
    if set(receipt) != expected_fields:
        raise ValueError(
            f"{boundary}: stacked post-PUF transfer aggregate receipt schema "
            f"changed; missing={sorted(expected_fields - set(receipt))}, "
            f"extra={sorted(set(receipt) - expected_fields)}."
        )
    _validate_stacked_post_puf_ownership_fields(receipt, boundary=boundary)


def _validate_stacked_post_puf_fit_records(
    value: object,
    *,
    boundary: str,
) -> list[dict[str, str]]:
    """Return exact JSON fit rows after validating the weights-audit schema."""

    if not isinstance(value, list):
        raise ValueError(f"{boundary}: fit-record audit is not a list.")
    ...
    for index, row in enumerate(value):
        if not isinstance(row, Mapping) or set(row) != {
            ...
            raise ValueError(
                f"{boundary}: fit-record audit row {index} has a non-canonical schema."
            )
        try:
            record = FitWeightRecord(
                fit_name=row["fit_name"],
                weight_kind=row["weight_kind"],
            )
        except (TypeError, ValueError) as error:
            raise ValueError(
                f"{boundary}: fit-record audit row {index} is invalid."
            ) from error
        validated.append(
            {
...
def validate_stacked_post_puf_transfer_receipt(
    ...
    *,
    ...
    _validate_stacked_post_puf_aggregate_ownership(receipt, boundary=boundary)
    ...
    expected_aggregate_fit_records: list[dict[str, str]] = []
    for name, group in expected_groups.items():
        ...
        if (
            ...
            or group_receipt.get("entity") != group.entity
            or group_receipt.get("family") != group.family
        ...
        if set(group_receipt) != expected_group_fields:
            raise ValueError(
                f"{boundary}: stacked post-PUF transfer group {name!r} receipt "
                "schema changed; "
                f"missing={sorted(expected_group_fields - set(group_receipt))}, "
                f"extra={sorted(set(group_receipt) - expected_group_fields)}."
            )
        if _json_ready(group_receipt["authority"]) != _json_ready(authority):
            raise ValueError(
                f"{boundary}: stacked post-PUF transfer group {name!r} authority "
                "differs from its aggregate authority."
            )
        _validate_stacked_post_puf_ownership_fields(
            group_receipt,
            boundary=f"{boundary} group {name!r}",
        )
        expected_aggregate_fit_records.extend(
            _validate_stacked_post_puf_fit_records(
                group_receipt["fit_records"],
                boundary=f"{boundary} group {name!r}",
            )
        )
        ...
        for target_key, target_receipt in group_targets.items():
            ...
            _validate_acs_transfer_origin_pattern_authority(
                target_receipt.get("origin"),
                expected_entity=group.entity,
                expected_family_targets=group.targets,
                boundary=f"{boundary} target {target_key!r} origin",
            )
            owner_receipt = target_receipt.get("post_transfer_calibration")
            ...
            if spec is None:
                ...
                if set(target_receipt) != expected_target_fields:
                    raise ValueError(
                        f"{boundary}: stacked post-PUF target {target_key!r} "
            ...
            if "qrf_pattern_evidence" in target_receipt:
                expected_target_fields.add("qrf_pattern_evidence")
            if set(target_receipt) != expected_target_fields:
                raise ValueError(
                    f"{boundary}: stacked post-PUF target {target_key!r} receipt "
                    "schema changed."
                )
    aggregate_fit_records = _validate_stacked_post_puf_fit_records(
        receipt["fit_records"],
        boundary=f"{boundary} aggregate",
    )
    if aggregate_fit_records != expected_aggregate_fit_records:
        raise ValueError(
            f"{boundary}: stacked post-PUF aggregate fit-record audit is not "
            "the canonical-order concatenation of its producer groups."
        )
    ...
    producer_rows_by_entity_role: dict[tuple[str, tuple[str, ...]], int] = {}
    for label, target_receipt in targets.items():
        ...
        entity, _family, target = label.split("/", 2)
        expected_roles = _canonical_post_puf_producer_roles(
            entity=entity,
            target=target,
        )
        observed_roles = target_receipt.get("producer_roles")
        if not isinstance(observed_roles, list) or observed_roles != list(
            expected_roles
        ):
            raise ValueError(
                f"{boundary}: stacked post-PUF transfer target {label!r} "
                f"producer roles are {observed_roles!r}, expected "
                f"{list(expected_roles)!r}."
            )
        producer_rows = target_receipt.get("producer_rows")
        if (
            isinstance(producer_rows, bool)
            or not isinstance(producer_rows, int)
            or producer_rows < 0
        ):
            raise ValueError(
                f"{boundary}: stacked post-PUF transfer target {label!r} has "
                f"invalid producer-row count {producer_rows!r}."
            )
        role_key = (entity, expected_roles)
        previous_producer_rows = producer_rows_by_entity_role.setdefault(
            role_key,
            producer_rows,
        )
        if producer_rows != previous_producer_rows:
            raise ValueError(
                f"{boundary}: stacked post-PUF transfer target {label!r} "
                f"producer-row count {producer_rows} differs from canonical "
                f"{entity}/{'+'.join(expected_roles)} sibling accounting "
                f"{previous_producer_rows}."
            )
        if frame is not None:
            expected_producer_rows = int(
                _post_puf_producer_mask(
                    frame,
                    entity=entity,
                    puf_produced="puf_clone" in expected_roles,
                    source_produced="asec_source" in expected_roles,
                ).sum()
            )
            if producer_rows != expected_producer_rows:
                raise ValueError(
                    f"{boundary}: stacked post-PUF transfer target {label!r} "
                    f"producer-row count {producer_rows} differs from the live "
                    f"canonical producer mask count {expected_producer_rows}."
                )
        counts: dict[str, int] = {}
        for count_name in (
            "authorized_null_rows",
    ...
    for name, group in expected_groups.items():
        ...
        canonical_qrf_catalog_sha256: str | None = None
        for target in group.targets:
            ...
            catalog_sha256 = _canonical_sha256(
                {"availability_patterns": origin["availability_patterns"]}
            )
            if canonical_qrf_catalog_sha256 is None:
                canonical_qrf_catalog_sha256 = catalog_sha256
            elif catalog_sha256 != canonical_qrf_catalog_sha256:
                raise ValueError(
                    f"{boundary}: active QRF targets in stacked post-PUF "
                    f"transfer group {name!r} disagree on their realized "
...
def _gap_fill_stacked_spine_evaluate(
    ...
    *,
    ...
    _validate_stacked_gap_fill_receipt(
        ...
        frame=current if production else None,
    )
