...
rotation-group files, fits the weighted Microcosm QRF for hourly wage and
...
tail was severe (microcosm#529).  The premium derivation deliberately deviates from the
...
adjudication receipts: microcosm#451 item 4 (overtime-incidence lane).

The upstream implementation cached the transformed monthly files as
``census_cps_org_2024_wages.csv.gz`` but did not publish that cache.  Microcosm
...
def fetch_org_2024_donor(
    ...
    *,
    ...
    root = (
        ...
        else Path.home() / ".cache" / "microcosm" / "org"
    )
...
def _recipient_features(frame: Frame, carried: pd.DataFrame) -> pd.DataFrame:
    ...
    # missing weeks pass income through untouched (microcosm#529). The
...
def derive_flsa_overtime_premium(
    *,
    ...
    receipts: microcosm#451 item 4 (overtime-incidence lane).
    """
