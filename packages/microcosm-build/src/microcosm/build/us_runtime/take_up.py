...
benefit and the reach of any eligibility-side reform (microcosm #312).

This module seeds the flags whose rate clears the provenance bar. The take-up
contract inventory (:mod:`microcosm.build.us_runtime.take_up_contract`) records,
per program, whether microcosm seeds it and the administrative source of its
...
the microcosm base spine, so the reported-receipt anchor the SNAP stage uses does
...
def us_take_up_participation_diagnostics(
    ...
    Records, per take-up flag, microcosm's weighted participation against the
    ...
    sibling workstream's zone (microcosm #313).
    """
    ...
    for program in contract.programs:
        ...
        if program.is_seeded and variable in live:
            ...
            # medicaid_take_up stage, microcosm #331); a non-constant column on
