...
microcosm issues #356/#368 (the dropped-column counterpart of #278's zeroed
input bases).
...
Microcosm issue #374 restores the retired enhanced-CPS source blend: each
...
earlier 2023 donor. See :mod:`microcosm.build.us_runtime.sipp_financial_assets`
...
left person-level SIPP predictions in place. Microcosm applies the requested
...
Pre-blend measured baseline (retained for comparison): the microcosm #368
...
1. The probe overlays assets onto a frame whose weights were already calibrated
   ...
   asset overlays on microcosm are documented to land at baseline 3.0-4.0M and a
...
until the signed sum equaled the direct ``scf_net_worth`` anchor. Microcosm does
...
def us_scf_wealth_signal_gate(
    ...
    *,
    ...
    the everyone-has-$0-resources failure of microcosm #356 (SSI resource-test
