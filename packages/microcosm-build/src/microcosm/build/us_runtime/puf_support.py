...
#: Finalization policies for cells the PUF pass does not own (microcosm#578
...
#: (microcosm #300). Stable so a release manifest and its allowlist can refer to
#: this fit by name.
...
# microcosm#516 donor mortgage quarantine: $10M of annual home-mortgage
...
#
# microcosm#567 measured that whole-row removal also deleted $98.176B of
...
# the outlier screen; never a legal requested output (microcosm#516).
...
# (partnership_self_employment_net_earnings, microcosm #432). These columns get
...
# added upstream is rejected even before anyone adds it here (microcosm issue
    ...
    *,
    ...
    Args:
        ...
        clone_attachment_fraction: Optional seeded-attachment fraction in
            ``(0, 1]`` (microcosm#578 revision item 3).  When set — assembled
...
def impute_us_puf_tax_detail_support(
    ...
    *,
    ...
    Args:
        fit_records: An optional sink for the build-level weights audit (microcosm
            #300). When a list is passed, this production fit appends one
            :class:`~microcosm.build.gates.FitWeightRecord`
            (``US_PUF_SUPPORT_FIT_NAME`` -> the kind the QRF *resolved* to,
            ``"design"`` here) so a release can prove the fit did not silently
            resolve unweighted (:func:`~microcosm.build.gates.weights_audit_gate`).
...
def prepare_us_puf_tax_detail_chain_inputs(
    ...
    *,
    ...
    switch (microcosm#578): recipient features are built null-preserving and a
...
def finalize_us_puf_tax_detail_predictions(
    ...
    *,
    ...
    clone arm (microcosm#578 audit item 1).  The legacy default reproduces the
    ...
    *,
    ...
    Args:
        ...
        engine: A metadata source exposing ``formula_owned_outputs(names) ->
            set[str]`` (a
            :class:`~microcosm.frame.adapters.policyengine_us.PolicyEngineUSVariableMetadataIndex`
...
def _require_complete_recipient_predictors(
    ...
    The microcosm#578 audit found the primary QRF silently zero-filling
...
def _ensure_float_output_column(
    ...
    *,
    ...
    (microcosm#578 audit item 1).  Preserve-nulls coercion is also
