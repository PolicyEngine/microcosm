...
Microcosm calibrates the US 2024 weights against SOI TY2022/TY2023 dollar
...
(PolicyEngine/microcosm#212, #116: AGI $16.0T vs CBO TY2025 ~$17T+; income tax
...
live in Microcosm as a named, versioned aging implementation that consumes
growth-factor facts from Ledger). This module is that Microcosm-side
...
microcosm#212 failure mode — unless the build explicitly waives it.
...
# The named, versioned Microcosm-side alignment model (PolicyEngine/ledger#71).
# Bump the version whenever the factor policy changes; no Ledger fact and no
# consumer-artifact hash changes when it does — only Microcosm build outputs.
...
class PeriodContractError(ValueError):
    ...
    This is the microcosm#212 guard, mirroring the Ledger consumption contract
    ...
    def __init__(self, violations: tuple[PeriodContractViolation, ...]) -> None:
        ...
        super().__init__(
            ...
            "(PolicyEngine/ledger#71; microcosm#212). Enable age_targets, "
    ...
    *,
    ...
    if "uprating_factor" in spec.metadata:
        ...
        if stamped is not None and _period_year(str(stamped)) is None:
            ...
            # included: they never age, but a microcosm-authored stamp that
