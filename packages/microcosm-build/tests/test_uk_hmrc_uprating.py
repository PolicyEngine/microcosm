...
def _spec(*, name: str, value: float, lower_bound: int | None, period: str = "2023"):
    metadata = {
...
def test_engine_index_refuses_a_fact_at_or_after_the_calibration_year() -> None:
    registry = TargetRegistry(
        (_spec(name="x", value=1.0, lower_bound=20_000, period="2025"),), country="uk"
    )

    with pytest.raises(ValueError, match="nothing to uprate"):
        align_hmrc_row_by_engine_index(
            _reference(EARNINGS_INDEX),
            registry,
            parameter_path=EARNINGS,
...
def test_count_growth_uses_the_containing_table_2_5_band_and_the_calendar_window() -> (
    ...
    assert forty.metadata["uprating_index_counts_by_opening_year"] == (
...
def test_every_spi_band_maps_to_table_2_5_bands_that_tile_the_range() -> None:
    ...
    for lower, bands in SPI_BAND_TO_ITL_BANDS.items():
        ...
        for (a_low, a_high), (b_low, _) in zip(bands, bands[1:], strict=False):
            assert a_high == b_low
