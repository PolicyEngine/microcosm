"""TRACE Transparent Research Object emission for microcosm builds.
...
from microcosm.build import trace

# A trimmed copy of the real
# packages/microcosm-data/build/us/release_manifest.json — same field names and
...
class TestCanonicalBytesAndFingerprint:
    ...
    def test_sha256_file_streams_correct_digest(self, tmp_path) -> None:
        payload = b"microcosm dataset bytes" * 1000
...
class TestBuildBuildTroDeterminism:
    ...
    def test_context_maps_all_required_namespaces(self) -> None:
        ...
        assert ctx["pop"] == "https://microcosm.institute/trace/0.1#"
    ...
    def test_tro_node_shape(self) -> None:
        ...
        assert node["trov:createdWith"]["schema:name"] == "microcosm-build"
        # microcosm-build version is pinned from the package
        assert (
            node["trov:createdWith"]["schema:softwareVersion"]
            == trace._microcosm_build_version()
        )
...
class TestReleaseManifestConverter:
    def _tro(self) -> dict:
        return trace.tro_from_release_manifest(
            ...
            input_artifacts=[PUF_INPUT],
            self_url="https://microcosm.institute/trace/us-2024.jsonld",
        )
    ...
    def test_build_identity_lands_on_performance(self) -> None:
        ...
        assert perf["pop:builder"] == "microcosm"
...
class TestEmissionContext:
    def test_github_actions_context(self, monkeypatch) -> None:
        ...
        assert perf["pop:ciRunUrl"] == (
            "https://github.com/PolicyEngine/microcosm/actions/runs/12345"
        )
