...
def test_uk_population_targets_shape_order_and_registry_accounting() -> None:
    ...
    assert len(resource["targets"]) == 296
    ...
    assert len(registry_scope) == 245
    assert len(profile_scope) == 51
    assert target_ids[:245] == registry_scope
    assert target_ids[245:] == profile_scope
    ...
    assert len(unmapped_declarations) == 53
