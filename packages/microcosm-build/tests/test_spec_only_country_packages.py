...
def test__given_greenfield_country_packages__then_each_uses_the_typed_schema() -> None:
    # Given
    country_roots = {root.name: root for root in _country_package_roots()}

    # When
    for country in ("am", "be", "nz"):
        manifest = json.loads(
            (country_roots[country] / "country_package.json").read_text(
                encoding="utf-8"
            )
        )
    ...
    assert {"am", "be", "nz"} <= set(country_roots)
