...
def test_exact_complete_ledger_has_one_primary_mode_per_pointer(field_ledger) -> None:
    assert len(field_ledger.fields) == EXPECTED_CONFIGURATION_FIELD_COUNT == 41_381
    assert field_ledger.source_counts == {
        "authored": 32_161,
        "resolved_bindings": 9_220,
    }
    ...
    assert len({field.pointer for field in field_ledger.fields}) == 41_381
