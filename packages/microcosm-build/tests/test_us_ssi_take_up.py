...
def test_stage_contract_pins_archived_method_and_band_structure() -> None:
    ...
    # microcosm#507/#508: the capacity basis may be a prior release's
    ...
    # registry (microcosm#469/#470) — the stage may never hardcode them.
...
def test_assignment_accepts_weight_split_puf_clone_indices() -> None:
    ...
    it (microcosm#568 review): a NEW household row carrying part of the
...
def test_every_band_saturated_stays_nonconstant_and_passes_the_gate() -> None:
    ...
    count (microcosm#469): the SSA-count miss is calibration's to close
    (microcosm#470) and ships in the scorecard.
    """
...
def test_existing_assignment_diagnostics_do_not_reassign_flags() -> None:
    ...
    # (microcosm#469/#470), reported in the scorecard, never a module failure.
...
def test_enforced_bands_are_the_adult_bands_pending_child_disability_stage() -> None:
    """The under-18 band is honestly fenced until microcosm#453/#509 lands.
...
def test_build_o_attempt_3_receipts_pin_the_floor_aware_stabilizer() -> None:
    """Regression fixture: Build O attempts 2/3 (microcosm#507, 2026-07-24).
...
def test_prior_basis_loader_accepts_current_and_legacy_artifacts() -> None:
    ...
    # (microcosm#507 Build O attempt 2).
...
def test_prior_basis_loader_rejects_invalid_artifacts(
    ...
    if mutation == "schema_1":
        ...
        # microcosm#463-class loop (microcosm#508 permits exactly one).
...
def test_delivery_gate_passes_within_tolerance_and_reports_the_fenced_band() -> None:
    ...
    delivered = _delivered(
        diagnostics,
        under_18=5.0,  # 90% miss — fenced, must not fail (microcosm#453/#509)
...
def test_delivery_gate_rejects_malformed_diagnostics() -> None:
    ...
    # matter how good the numbers look (microcosm#507 sol review finding 1).
...
def test_delivery_gate_fences_override_enforcement_with_their_adjudication() -> None:
    """microcosm#566/#567: the dense arm fences its adult bands.
    ...
    fences = {
        "18_64": "Fenced for the dense diagnostic arm (microcosm#566/#567).",
        "65_plus": "Fenced for the dense diagnostic arm (microcosm#566/#567).",
    }
    ...
    # keeps its own microcosm#453/#509 fence untouched.
...
def test_delivery_gate_partial_fence_keeps_the_other_band_enforced() -> None:
    _, _, _, diagnostics = _assigned()
    fences = {"65_plus": "Fenced for the dense diagnostic arm (microcosm#566)."}
