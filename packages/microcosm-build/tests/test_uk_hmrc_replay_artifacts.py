...
def test_promoted_145_column_gate_passes_with_required_spi_support() -> None:
    ...
    assert details["required_columns"] == 145
    assert details["reviewed_exclusions"] == {}
    assert details["stale_exclusions"] == []
    ...
    build_state = details["family_build_state"]["hmrc_spi_income"]
    assert build_state["actual_output_weight_kind"] == "importance"
    assert build_state["actual_build_period"] == "2023"
    assert build_state["valid_mass_change_records"] >= 1


def test_national_staging_record_is_aggregate_and_binds_green_artifacts() -> None:
    ...
    assert record["input_coverage"]["missing"] == []
    assert record["input_coverage"]["degenerate_required"] == []
    assert record["input_coverage"]["insufficient_effective_mass"] == []
    assert record["dataset"]["household_weight_kind"] == "importance"
    assert record["dataset"]["entity_rows"]["person"] > 0
    assert record["dataset"]["entity_rows"]["household"] > 0
    assert record["artifacts"]["staging_h5"]["retention"] == "local_untracked"
    assert record["artifacts"]["spi_donor"] == {
        "sha256": SPI_DONOR_SHA256,
        "size_bytes": SPI_DONOR_SIZE_BYTES,
    }
    assert record["artifacts"]["hmrc_surface"]["sha256"] == (
        HMRC_SPI_COLLATED_ODS_SHA256
    )
    assert record["hmrc_replay"]["summary"]["excluded_with_fence"] == (
        HMRC_SPI_TARGET_RECORD_COUNT
    )
...
def test_committed_replay_artifacts_contain_no_row_level_payloads_or_local_paths() -> (
    ...
    for resource in (_REPLAY_RESOURCE, _GATE_RESOURCE, _BUILD_RECORD_RESOURCE):
