...
class TestExportedNonzeroGate:
    def test_all_zero_column_fails_with_remedy_named(self) -> None:
        from microcosm.build import exported_nonzero_gate
...
class TestDefaultValuedColumnsGate:
    ...
    def test_reviewed_exclusion_accepts_known_degenerate_column(self) -> None:
        result = default_valued_columns_gate(
            ...
            {"weekly_hours_worked_before_lsr": 40.0},
            reviewed_exclusions={
                "weekly_hours_worked_before_lsr": "tracked in microcosm#242"
            },
        )
        assert result.passed
...
class TestColumnImplicationGate:
    def test_positive_numeric_rows_require_true_boolean(self) -> None:
        result = column_implication_gate(
            [0.0, 10.0, -1.0, 5.0],
            [False, True, False, False],
            numeric_column="benefit_reported",
            boolean_column="would_claim",
        )

        assert not result.passed
        assert result.details["implicated_rows"] == 2
        assert result.details["violation_count"] == 1
        assert "benefit_reported > 0" in result.failures[0]

    def test_zero_and_negative_rows_do_not_imply_boolean(self) -> None:
        result = column_implication_gate(
            [0.0, -1.0, 1.0],
            [False, False, True],
            numeric_column="benefit_reported",
    ...
    def test_evidence_must_be_aligned_and_boolean(self) -> None:
        ...
        with pytest.raises(ValueError, match="boolean or integer 0/1"):
            column_implication_gate(
                [1.0],
                [2],
                numeric_column="x",
...
class TestFitWeightRecord:
    def test_accepts_the_resolved_weight_kinds(self) -> None:
        ...
        # explicit (a DataFrame vector fit) — the vocabulary microcosm.fit's
...
#: (microcosm#462): the +634.8% capital-gain-distributions defect and the
...
class TestTargetFitGate:
    ...
    def test_reviewed_exclusion_passes_and_is_recorded(self) -> None:
        result = target_fit_gate(
            ...
            (_TABLE_1_4_REQUIREMENT,),
            reviewed_exclusions={
                _BUILD_M_CGD_ROW["name"]: "microcosm#462: tracked QRF defect."
            },
        )
        assert result.passed
        assert result.details["reviewed_exclusions"] == {
...
class TestTailConcentrationGate:
    def _build_m_signature(self) -> tuple[np.ndarray, np.ndarray]:
        """The microcosm#462 non_sch_d_capital_gains tail point-mass, to scale.
    ...
    def test_reviewed_exclusion_passes_and_is_recorded(self) -> None:
        ...
        result = tail_concentration_gate(
            ...
            {"non_sch_d_capital_gains": weights},
            reviewed_exclusions={
                "non_sch_d_capital_gains": "microcosm#462: Build N QRF fix owns this."
            },
        )
        assert result.passed
        assert result.details["reviewed_exclusions"] == {
