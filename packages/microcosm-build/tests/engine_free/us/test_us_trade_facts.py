...
GOLDEN = _TEST_PATHS.tests / "golden" / "us_trade"
...
def test_fact_rows_are_deterministic_and_carry_producer_identity():
    ...
    for row in first:
        ...
        assert "microcosm-minted" in row["source"]["extraction_method"]
...
def test_artifact_round_trips_through_the_microcosm_loader(tmp_path):
    rows = build_import_entry_fact_rows(
        _margins(),
        retrieval_manifest=_manifest_entries(),
        extracted_at="2026-08-05T00:00:00+00:00",
    ...
    assert (
        ...
        == "microcosm.build.us_runtime.us_trade"
    )
...
def test_facts_compile_into_microcosm_ledger_targets(tmp_path):
    rows = build_import_entry_fact_rows(
        _margins(),
        retrieval_manifest=_manifest_entries(),
        extracted_at="2026-08-05T00:00:00+00:00",
