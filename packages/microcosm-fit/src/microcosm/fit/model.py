...
A conditional model fits ``P(y | x)`` over a :class:`~microcosm.frame.Frame` —
...
:mod:`microcosm.fit` routes its weight handling through them, so "no silent
...
from microcosm.frame import Frame, WeightKind, assert_kind_transition
...
#: default: a microcosm fit is weighted unless the caller opts out.
...
#: or vector: weighted, but with no kernel :class:`~microcosm.frame.WeightKind`
...
class FittedModel(Protocol):
    ...
    def predict(self, frame_or_df: Frame | pd.DataFrame) -> pd.DataFrame:
        ...
        Args:
            frame_or_df: The rows to impute for. A :class:`~microcosm.frame.Frame`
...
class ConditionalModel(Protocol):
    ...
    slot in behind it later (DESIGN.md, "microcosm-fit: conditional models").
        ...
        *,
        ...
        Args:
            frame_or_df: The rows to fit on. For a
                :class:`~microcosm.frame.Frame`, predictors and targets must all
            ...
            weights: On a Frame: which typed weight vector of the owning entity
                to weight the fit by — a :class:`~microcosm.frame.WeightKind` or
...
def resolve_fit_weights(
    ...
    This is the enforcement point for the operator's defining rule: a microcosm
    ...
    Weights are resolved through :meth:`~microcosm.frame.Frame.resolve_weights`,
    ...
    Args:
        ...
        weights: ``"none"`` to fit unweighted (the only unweighted path), or a
            :class:`~microcosm.frame.WeightKind` / its string value selecting
    ...
    Raises:
        TypeError: If ``weights`` is neither a string nor a
            :class:`~microcosm.frame.WeightKind`.
...
def resolve_dataframe_fit_weights(
    ...
    *,
    ...
    / a :class:`~microcosm.frame.WeightKind`) are Frame concepts and are refused
...
def resolved_weight_kind(
    ...
    (possibly inherited) :class:`~microcosm.frame.WeightKind`, so a person-level
    ...
    value a fitted model records for the build-level weights audit (microcosm
    ...
    Returns:
        ...
        resolved :class:`~microcosm.frame.WeightKind` value
