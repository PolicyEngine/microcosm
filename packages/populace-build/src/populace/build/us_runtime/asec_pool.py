...
class PooledAsecSource:
    """Pooled ASEC person frame plus household-table-aligned weights."""
...
def pool_asec_sources(
    ...
    *,
    ...
    declares shares, every year receives equal share. When ``anchor_year`` is
    provided, total population is anchored to that source year's weighted
    person population; otherwise it is anchored to the first source.
    """

    source_list = tuple(sources)
    if not source_list:
        raise ValueError("At least one ASEC source is required.")
    _require_unique_source_years(source_list)
    shares = _resolved_shares(source_list)
    raw_inputs = [
        _prepare_year_input(source, weight_scale=weight_scale) for source in source_list
    ]
    target_population = _target_person_population(
        source_list,
        raw_inputs,
    ...
    pooled_person = _globalize_source_unit_column(pooled_person, "SPM_ID")
    metadata = {
        "anchor_year": anchor_year,
    ...
    return PooledAsecSource(
        ...
        household_weights=_household_weights_for_pooled_person(pooled_person),
        metadata=metadata,
    )
...
def build_pooled_asec_unit_frame(
    ...
    *,
    ...
    pooled = pool_asec_sources(sources, anchor_year=target_year)
    strata = pd.Series(
    ...
    frame = assign_us_unit_structure(
        ...
        year=target_year,
        household_weights=pooled.household_weights,
        schema=US_SCHEMA,
...
def _target_person_population(
    sources: tuple[AsecSource, ...],
    raw_inputs: list[dict[str, Any]],
    *,
    anchor_year: int | None,
) -> float:
    if anchor_year is None:
        return float(raw_inputs[0]["raw_person_population"])
    matches = [
        index for index, source in enumerate(sources) if source.year == anchor_year
    ]
    if not matches:
        available = ", ".join(str(source.year) for source in sources)
        raise ValueError(
            f"ASEC anchor year {anchor_year} is not among source years: {available}."
        )
    if len(matches) > 1:
        raise ValueError(f"ASEC anchor year {anchor_year} appears more than once.")
    return float(raw_inputs[matches[0]]["raw_person_population"])


def _require_unique_source_years(sources: tuple[AsecSource, ...]) -> None:
    seen: set[int] = set()
    duplicates: set[int] = set()
    for source in sources:
        if source.year in seen:
            duplicates.add(source.year)
        seen.add(source.year)
    if duplicates:
        years = ", ".join(str(year) for year in sorted(duplicates))
        raise ValueError(
            f"ASEC source years must be unique; duplicate year(s): {years}."
        )
...
def _globalize_source_unit_column(person: pd.DataFrame, column: str) -> pd.DataFrame:
    ...
    keys = pd.MultiIndex.from_arrays(
        [
            result.loc[~missing, "source_year"].to_numpy(),
            values.loc[~missing].to_numpy(),
        ],
        names=["source_year", column],
    )
    codes, _uniques = pd.factorize(keys, sort=True)
    if missing.any():
        result[column] = result[column].astype("object")
        result.loc[~missing, column] = codes.astype("int64") + 1
    else:
        result[column] = codes.astype("int64") + 1
    return result


def _household_weights_for_pooled_person(person: pd.DataFrame) -> Weights:
    weights = person.groupby("PH_SEQ", sort=True)[ASEC_PERSON_WEIGHT_COLUMN]
    low = weights.min()
    high = weights.max()
    unequal = ~np.isclose(low.to_numpy(), high.to_numpy())
    if unequal.any():
        bad = low.index.to_numpy()[unequal][:5].tolist()
        raise ValueError(
            "Pooled ASEC household weights must be constant within each "
            f"household; household id(s) {bad} carry unequal values."
        )
    return Weights(low.to_numpy(dtype=np.float64), WeightKind.DESIGN)
