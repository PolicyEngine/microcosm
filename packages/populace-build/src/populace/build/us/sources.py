"""US primary-source imputation stages — the canonical implementations.

Each function is one donor stage of the US build (the `us_plan` donor graph):
Fed SCF 2022 wealth, SIPP tips and vehicles, CPS-ORG labor-market inputs,
MEPS-IC employer-premium parameters, prior-year ASEC income, mortgage
conversion from SCF hints + PUF interest, and ACS rent. Donor loaders import
lazily from policyengine-us-data (install the ``us`` extra), so the base
shard stays light and these functions fail loudly at call time when a donor
is unavailable — never silently.

These currently operate at the legacy driver's person/household DataFrame
grain; the Frame-native ``us_plan(implementations)`` wiring replaces that
calling convention when the engine swap lands (the stage *logic* is already
canonical here).
"""
...
def add_scf_wealth(
    person: pd.DataFrame, hh: pd.DataFrame, seed: int, log
) -> pd.DataFrame:
    ...
    pf = pd.DataFrame(
        {
            ...
            "is_married": person.get("A_MARITL", pd.Series(0, index=person.index))
            .isin([1, 2])
            .astype(float)
            ...
            "interest_dividend_income": num("taxable_interest_income")
            + num("dividend_income")
            + num("qualified_dividend_income")
            + num("non_qualified_dividend_income"),
    ...
    def hh_sum(cols):
        ...
        return (
            sum(hh[c].to_numpy(dtype=np.float64) for c in present) if present else None
        )
    ...
    log(
        f"  SCF wealth block: {len(targets)} scf vars + net_worth + {sorted(head_carry_to_person)} (weighted=wgt)"
    )
    return person, hh
...
def add_sipp_tips(person: pd.DataFrame, log) -> pd.DataFrame:
    ...
    x["age"] = pd.to_numeric(
        person.get("age", person.get("A_AGE", 0)), errors="coerce"
    ).fillna(0)
    ...
    for c in needed:
        if c not in x.columns:
            src = person.get(c)
            x[c] = (
                pd.to_numeric(src, errors="coerce").fillna(0)
                if src is not None
                else 0.0
            )
    try:
        ...
        person.loc[
            ~person.get(
                "is_tipped_occupation", pd.Series(False, index=person.index)
            ).astype(bool),
            "tip_income",
        ] = 0.0
...
def add_org_wages(
    person: pd.DataFrame, hh: pd.DataFrame, year: int, log
) -> pd.DataFrame:
    ...
    cps.update(
        {
            ...
            .fillna(0)
            .to_numpy(np.float32),
            "is_female": person.get("is_female", pd.Series(False, index=person.index))
            .astype(bool)
            .to_numpy(),
            "cps_race": pd.to_numeric(person.get("cps_race", 0), errors="coerce")
            .fillna(0)
            .to_numpy(np.float32),
            "weekly_hours_worked": pd.to_numeric(
                person.get("hours_worked_last_week", 0), errors="coerce"
            )
            .fillna(0)
            .to_numpy(np.float32),
            "hours_worked_last_week": pd.to_numeric(
                person.get("hours_worked_last_week", 0), errors="coerce"
            )
            .fillna(0)
            .to_numpy(np.float32),
            "weeks_worked": pd.to_numeric(
                person.get("weeks_worked", 0), errors="coerce"
            )
            .fillna(0)
            .to_numpy(np.float32),
            "is_hispanic": person.get(
                "is_hispanic", pd.Series(False, index=person.index)
            )
            .astype(bool)
            .to_numpy(),
        }
...
def add_prior_year_income(person: pd.DataFrame, asec_year: int, log) -> pd.DataFrame:
    ...
    emp = (
        cur_ids.map(prior["WSAL_VAL"])
        if "WSAL_VAL" in prior.columns
        else pd.Series(np.nan, index=person.index)
    )
    se = (
        cur_ids.map(prior["SEMP_VAL"])
        if "SEMP_VAL" in prior.columns
        else pd.Series(np.nan, index=person.index)
    )
    matched = emp.notna() & se.notna() & ~emp.isin(sentinels) & ~se.isin(sentinels)
    person["employment_income_last_year"] = (
        pd.to_numeric(emp, errors="coerce").where(matched, 0.0).fillna(0.0)
    )
    person["self_employment_income_last_year"] = (
        pd.to_numeric(se, errors="coerce").where(matched, 0.0).fillna(0.0)
    )
...
def add_mortgage_conversion(
    ...
    def person_col(name, default=0.0):
        return (
            pd.to_numeric(person.get(name, default), errors="coerce")
            .fillna(0.0)
            .to_numpy(np.float32)
        )
    ...
    data.update(
        {
            ...
            "is_female": {
                tp: person.get("is_female", pd.Series(False, index=person.index))
                .astype(bool)
                .to_numpy()
            },
            "cps_race": {tp: person_col("cps_race")},
            "is_tax_unit_head": {
                tp: (person.get("tax_unit_role_input", "") == "HEAD").to_numpy()
            },
            "is_tax_unit_spouse": {
                tp: (person.get("tax_unit_role_input", "") == "SPOUSE").to_numpy()
            },
            ...
            "tenure_type": {
                tp: hh.get("tenure_type", pd.Series("NONE", index=hh.index))
                .fillna("NONE")
                .astype(str)
                .to_numpy()
            },
    ...
    hh_tenure = dict(zip(hh["household_id"], data["tenure_type"][tp], strict=True))
    spm_ids = data["spm_unit_id"][tp]
    spm_hh = (
        pd.DataFrame(
            {"spm": person["person_spm_unit_id"], "hh": person["person_household_id"]}
        )
        .drop_duplicates("spm")
        .set_index("spm")["hh"]
    )
    data["spm_unit_tenure_type"] = {
        tp: np.array(
    ...
    for key in set(data) - before | {"interest_deduction"}:
        ...
        if len(arr) == len(person):
            ...
            raise ValueError(
                f"mortgage conversion output {key!r} has odd length {len(arr)}"
            )
    log(f"  mortgage conversion: person outputs + head-carried {sorted(tu_outputs)}")
    inv = person.get("investment_interest_expense")
    if inv is not None:
        log(
            f"  investment_interest_expense: nz {(pd.to_numeric(inv, errors='coerce').fillna(0) > 0).mean() * 100:.1f}%"
        )
    return person
...
def add_acs_rent(person: pd.DataFrame, hh: pd.DataFrame, seed: int, log):
    ...
    recv = pd.DataFrame(
        {
            "hh_employment_income": pd.to_numeric(
                person["employment_income"], errors="coerce"
            )
            .fillna(0)
    ...
    recv["state_fips"] = (
        pd.to_numeric(hh.get("state_fips", 0), errors="coerce").fillna(0).to_numpy()
    )
    draws = fitted.predict(recv[pred].reset_index(drop=True))

    hh = hh.copy()
    rent = _support_guard(
        np.asarray(draws["rent"], dtype=np.float64),
...
def add_vehicle_assets(person: pd.DataFrame, hh: pd.DataFrame, log):
    ...
    hh = hh.copy()
    if rid is not None:
        owned_by = dict(zip(rid.tolist(), owned.tolist(), strict=True))
        value_by = dict(zip(rid.tolist(), value.tolist(), strict=True))
    ...
    if "net_worth" in hh.columns:
        hh["net_worth"] = hh["net_worth"].to_numpy(dtype=np.float64) + hh[
            "household_vehicles_value"
