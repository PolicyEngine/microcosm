...
import populace.build.stage_profile as stage_profile
...
def test__given_failing_stage__then_failure_is_recorded_and_reraised(
    ...
    assert not any(
        thread.name == "populace-stage-profile-b" and thread.is_alive()
...
def test__given_exit_rss_failure__then_sampler_thread_is_still_cleaned_up(
    ...
    # Then
    assert not any(
        thread.name == "populace-stage-profile-c" and thread.is_alive()
        for thread in threading.enumerate()
    )
