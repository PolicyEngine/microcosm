...
def _base_frame(
    *,
    ...
    tables = {
        ...
        "household": pd.DataFrame({"household_id": [1, 2], "state_fips": [1, 1]}),
...
def test__given_mixed_spines__then_ladder_keeps_acs_puma_and_assigns_asec() -> None:
    # Given
    base = _base_frame()
    acs = _acs_frame()
    ...
    asec_rows = household[household[spine_column("household")].eq(ASEC_PUF_SPINE)]
...
def _assigned_geography_base_frame(
    *,
    ...
    """A base frame whose donor households carry block-ladder geography."""

    base = _base_frame()
    tables = {entity: base.table(entity).copy() for entity in base.entities}
    household = tables["household"]
    household["tract_geoid"] = tracts or ["01001000100", "01003000100"]
    household["congressional_district_geoid"] = congressional_districts or [
        101,
        102,
    ]
    household["county_fips"] = counties or ["01001", "01003"]
    for column in drop_columns or []:
        household.drop(columns=[column], inplace=True)
    return Frame(
        tables,
        US_SCHEMA,
...
def test__given_partial_donor_assigned_geography_columns__then_pool_fails() -> None:
    base = _assigned_geography_base_frame(
        drop_columns=["congressional_district_geoid", "county_fips"],
    )

    with pytest.raises(ValueError, match="partial assigned-geography"):
        with_optional_acs_spine(
            base,
            _acs_frame(),
            puma_ladder=_test_puma_ladder(),
...
def test__given_partially_empty_donor_assigned_geography__then_pool_fails() -> None:
    base = _assigned_geography_base_frame(
        tracts=["01001000100", ""],
    )

    with pytest.raises(ValueError, match="partially assigned geography"):
        with_optional_acs_spine(
            base,
            _acs_frame(),
            puma_ladder=_test_puma_ladder(),
...
def test__given_donor_tract_absent_from_ladder__then_pool_fails() -> None:
    base = _assigned_geography_base_frame(
