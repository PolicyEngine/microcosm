...
SCHEMA_MIGRATION = (
    REPOSITORY_ROOT / "supabase" / "migrations" / "20260805000000_chronicle.sql"
)
MIGRATION = (
    REPOSITORY_ROOT
    / "supabase"
...
def test_prediction_schema_accepts_claims_and_resolution_events() -> None:
    ...
    predictions = sql.split("CREATE TABLE chronicle.predictions (", 1)[1].split(
        "\n);", 1
    )[0]
    ...
    assert "REFERENCES chronicle.predictions (id)" in predictions
