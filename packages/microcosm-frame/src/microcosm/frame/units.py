...
unit systems, and returns a :class:`~microcosm.frame.bundle.Frame` whose
...
(install via ``microcosm-frame[us]``). This module never re-implements those
...
def assign_us_unit_structure(
    ...
    * **marital unit**: a person is paired with their spouse when
    ...
    Returns:
        A validated :class:`~microcosm.frame.bundle.Frame` with the
...
def _import_construct_tax_units():
    ...
    try:
        ...
        raise ImportError(
            ...
            "'microcosm-frame[us]' to use assign_us_unit_structure()."
        ) from exc
...
def _assign_spm_unit_ids(
    ...
    partition = assign_spm_partition(
        spm_input,
        family_col="__microcosm_frame_no_family_fallback__",
    )
