...
class TestLazyImport:
    ...
    def test_engine_methods_name_the_extra_when_missing(self, monkeypatch) -> None:
        real_import = builtins.__import__

        def import_without_policyengine_us(name, *args, **kwargs):
            if name == "policyengine_us":
                raise ModuleNotFoundError(name)
            return real_import(name, *args, **kwargs)

        monkeypatch.setattr(builtins, "__import__", import_without_policyengine_us)
        adapter = PolicyEngineUSEngine()
        with pytest.raises(ImportError, match=r"microcosm-frame\[policyengine\]"):
