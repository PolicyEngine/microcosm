...
import populace.fit
from populace.fit import __version__ as fit_version
from populace.fit import _assert_frame_compatible


def test_namespace_has_no_top_level_init() -> None:
    """populace stays a PEP 420 namespace; the fit shard ships no __init__ for it.

    A shard clobbering ``populace/__init__.py`` would break side-by-side install
    of populace-frame and populace-calibrate. (The fit *subpackage* has its own
...
def test_fit_declares_its_own_version() -> None:
    ...
    assert populace.fit.__version__ == "0.1.0"
