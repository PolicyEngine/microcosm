"""Resolve, verify, download, and load a certified populace release.
...
from populace.data.contract import RELEASE_MANIFEST_SCHEMA_VERSION
from populace.data.registry import DEFAULT_VARIANT, REGISTRY, DatasetSpec
from populace.data.release import latest_release
...
def _certified_release_from_manifest(
    ...
    if schema_version != RELEASE_MANIFEST_SCHEMA_VERSION:
        raise ValueError(
            ...
            f"{RELEASE_MANIFEST_SCHEMA_VERSION}. Upgrade populace-data."
        )
    build = _required_mapping(manifest, "build", release_id=release_id)
