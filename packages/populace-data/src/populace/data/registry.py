...
(:mod:`populace.data.loader`) reads this registry; nothing else hard-codes a
...
``populace-data`` pulls neither torch nor any country engine until a load
actually needs one — country engines are optional extras (``populace-data[us]``).
"""
...
class DatasetSpec:
    """A pointer to one published populace population and how to read it.
