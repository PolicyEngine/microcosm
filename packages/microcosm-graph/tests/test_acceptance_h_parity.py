...
#: H2: ``uk_spine.json`` — the 29-stage FRS spine expressed as a graph — plus
...
def test_h2_uk_spine_parity(tmp_path: Path) -> None:
    ...
    ``uk_spine.json`` and ``sources/``; both sides run the 29 transforms from
    ...
    in ``tools/build_uk_frs_spine.py`` — the 29 names intersected with a
    29-stage packaged manifest, kept in step by hand — can be deleted.
    """
    ...
    assert len(compiled.order) >= 30, "a CREATE node plus the 29 spine stages"
    assert all(
